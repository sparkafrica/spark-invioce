...
export function MemoForm({ initialData, isEditing = false, memoId, onCancel, onSuccess }: MemoFormProps) {
  const form = useForm({
    defaultValues: {
      number: '',
    ...
    validators: {
      onChange: ({ value }) => standardSchemaValidators.validate({ value, validationSource: 'field' }, memoSchema),
      onSubmit: ({ value }) => standardSchemaValidators.validate({ value, validationSource: 'form' }, memoSchema),
    },
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto max-w-[860px] flex flex-col gap-6 py-6" style={{ padding: '28px 24px 56px' }}>
          ...
          <form.Subscribe selector={(s) => [s.canSubmit, s.isSubmitting]}>
            {([canSubmit, isSubmitting]) => (
              <Button type="submit" variant="default" disabled={!canSubmit || isSubmitting} className="flex-1 bg-[#ec3013] text-white border border-[#ec3013] py-2.5 text-xs font-semibold hover:bg-[#c02a10] disabled:opacity-50 rounded-none">
                {isSubmitting ? 'Saving…' : isEditing ? 'Save changes' : 'Create memo'}
              </Button>
            )}
          </form.Subscribe>
        </div>
      ...
      <div className="border-2 border-[#201e1d] bg-white p-4 flex flex-col gap-4">
        <div className="text-[10px] tracking-[0.12em] font-semibold text-[#c02a10]">MEMO — {isEditing ? 'EDIT' : 'NEW'}</div>
          ...
          <form.Field name="number">
            {(field) => (
              <Field>
                ...
                <Input id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="MEMO-2026-001" />
          ...
          <form.Field name="to">
            {(field) => (
              <Field>
                ...
                <Input id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="B4B Partners Limited — Chinapa Onwusah" />
                <FieldError errors={field.state.meta.errors} />
              </Field>
            )}
          </form.Field>
          <form.Field name="from">
            {(field) => (
              <Field>
                ...
                <Input id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="Nnaemeka Clinton, CEO" />
                <FieldError errors={field.state.meta.errors} />
              </Field>
            )}
          </form.Field>
        ...
        <form.Field name="companyId">
          {(field) => (
            <Field>
              <FieldLabel htmlFor={field.name}>Invoicing company on the letterhead</FieldLabel>
              <Select value={field.state.value} onValueChange={(v) => field.handleChange(v ?? '')}>
        ...
        <form.Field name="subject">
          {(field) => (
            <Field>
              ...
              <Input id={field.name} name={field.name} value={field.state.value} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} placeholder="Payment schedule and milestone acceptance" />
              <FieldError errors={field.state.meta.errors} />
            </Field>
          )}
        </form.Field>
        ...
        <form.Field name="body">
          {(field) => (
            <Field>
              <FieldLabel htmlFor={field.name}>Body — one paragraph per line</FieldLabel>
              <Textarea id={field.name} name={field.name} value={field.state.value || ''} onBlur={field.handleBlur} onChange={(e) => field.handleChange(e.target.value)} rows={14} placeholder="This memo accompanies invoice SPK-2026-0812.\n\nThe fee of N25,500,000.00 excluding VAT is payable across four milestones..." />
              <FieldError errors={field.state.meta.errors} />
            </Field>
          )}
        </form.Field>
