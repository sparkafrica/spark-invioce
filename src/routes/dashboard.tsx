			...
			<div className="grid lg:grid-cols-[1.45fr_1fr] gap-0.5 bg-[#201e1d] border-2 border-[#201e1d]">
				<div className="bg-white p-4 lg:p-5">
					...
					{filtered.length === 0 ? (
						...
						<div className="h-45 bg-[#faf9f9] border border-[#e7e4e2] flex items-center justify-center text-xs text-[#5c5755]">
							Trend — {filtered.length} invoices · invoiced{' '}
							{fmtShort(totals.invoiced)} · collected{' '}
					...
					<div className="flex justify-between items-baseline gap-2.5 mb-3.5">
						...
						<div className="flex gap-1">
							<Button
								...
								variant="default"
								size="sm"
								className="bg-[#201e1d] text-white border border-[#201e1d] px-2.5 py-1.5 text-xs font-semibold rounded-none"
							>
								Count
							</Button>
							<Button
								...
								variant="outline"
								size="sm"
								className="bg-white text-[#201e1d] border border-[#201e1d] px-2.5 py-1.5 text-xs font-semibold hover:bg-[#f0dcd8] rounded-none"
							>
								Value
							</Button>
						</div>
							...
							{[
								{
									name: 'Paid',
									count: String(
										filtered.filter((i: any) => i.status === 'paid').length,
									),
									pct: totals.total
										? `${Math.round((filtered.filter((i: any) => i.status === 'paid').length / totals.total) * 100)}%`
										: '0%',
									color: '#201e1d',
								},
								{
									name: 'Part paid',
									count: String(
										filtered.filter((i: any) => i.status === 'part_paid')
											.length,
									),
									pct: totals.total
										? `${Math.round((filtered.filter((i: any) => i.status === 'part_paid').length / totals.total) * 100)}%`
										: '0%',
									color: '#f7d9d3',
								},
								{
									...
									count: String(
										filtered.filter(
											(i: any) => !['paid', 'part_paid'].includes(i.status),
										).length,
									),
									pct: totals.total
										? `${Math.round((filtered.filter((i: any) => !['paid', 'part_paid'].includes(i.status)).length / totals.total) * 100)}%`
										: '0%',
									color: '#fff',
									border: true,
								},
								...
								>
									<span
										...
										style={{
											background: d.color,
											borderColor: (d as any).border ? '#201e1d' : d.color,
										}}
									/>
									<span>{d.name}</span>
				...
				<div className="bg-white p-4 lg:p-5">
					...
					<div className="flex flex-col gap-3.5">
						{businessNames.map((b) => {
							const bizInvs =
								biz === 'All'
									? invoices.filter((i: any) => i.business === b)
									: filtered.filter((i: any) => i.business === b);
				...
				<Table className="bg-white">
					...
					<TableBody>
						{businessNames.map((b) => {
							...
							const ngn = bInvs
								.filter((i: any) => (i.currency || 'NGN') === 'NGN')
								.reduce(
									(s: number, i: any) =>
										s + (Number(String(i.total).replace(/[^0-9.-]/g, '')) || 0),
									0,
							...
							const kes = bInvs
								.filter((i: any) => (i.currency || 'NGN') === 'KES')
								.reduce(
									(s: number, i: any) =>
										s + (Number(String(i.total).replace(/[^0-9.-]/g, '')) || 0),
									0,
