...
import { useBanks, useBusinesses, useClients, useCompanies, useProducts } from '#/hooks/useReferences'
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {businessesData?.businesses?.map((b: any) => (
                          <CommandItem key={b.id} value={b.name} onSelect={() => field.handleChange(b.id)} className="rounded-none" data-checked={field.state.value === b.id}>
                            {b.name}
                          </CommandItem>
                        ))}
                      </CommandGroup>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {companiesData?.companies?.map((c: any) => (
                          <CommandItem key={c.id} value={c.name} onSelect={() => field.handleChange(c.id)} className="rounded-none" data-checked={field.state.value === c.id}>
                            {c.name}
                          </CommandItem>
                        ))}
                      </CommandGroup>
        ...
        <form.Field name="currency">
          {(field) => (
            <Field>
              ...
              <CurrencySelect value={field.state.value} onValueChange={(v) => field.handleChange(v)} placeholder="Select a currency" />
              <FieldError errors={field.state.meta.errors} />
            </Field>
          )}
        </form.Field>
    ...
    <>
      ...
      <div className="grid grid-cols-[1fr_1.4fr] gap-4 items-start">
        <div className="space-y-2">
          ...
          <Button type="button" onClick={() => setShowNewClientDialog(true)} size="sm" className="self-start border border-[#201e1d] text-black bg-transparent w-min text-xs hover:bg-[#f0dcd8]">+ New client</Button>
        </div>
        <form.Field name="clientId">
          {(field) => {
            ...
            return (
              <div className="border-l-2 border-[#201e1d] pl-3.5 text-xs leading-5 mt-3">
                ...
                <div className="text-[#5c5755]">{selected.contact || ''} — {selected.email || '—'}</div>
              </div>
            )
          }}
        </form.Field>
        ...
        <form.Field name="items" mode="array">
          {(field) => (
            <div className="flex gap-2">
              <Button type="button" onClick={() => field.pushValue({ name: '', description: '', qty: '1', cost: '0', discountName: '', discountPct: '0', discountAmt: '0', sortOrder: field.state.value.length })} size="sm" className="border border-[#201e1d] bg-white text-xs font-semibold hover:bg-[#f0dcd8] rounded-none text-[#201e1d]">Add blank line</Button>
              {field.state.value.length > 1 && (
                <Button type="button" size="sm" onClick={() => {
                  ...
                  className="border border-[#201e1d] bg-white text-xs font-semibold hover:bg-[#f0dcd8] rounded-none text-[#201e1d]">Split evenly</Button>
              )}
            </div>
          )}
        </form.Field>
        ...
        <div>
          <div className="text-[10px] tracking-[0.12em] font-semibold text-[#c02a10] mb-3">PAYMENT TERMS</div>
            <div className="flex gap-2 mb-3">
              <Button
                type="button"
                variant={pTypeField.state.value !== 'tranche' ? 'default' : 'outline'}
            ...
            <div className="text-xs text-[#5c5755]">One payment, due on the due date above. Record what has been received on the invoice itself, under Payment status.</div>
          ) : (
            <form.Field name="tranches" mode="array">
              {(field) => (
                <>
                  <div className="flex gap-2 mb-3">
                    <Button type="button" onClick={() => field.pushValue({ name: `M${field.state.value.length + 1} — `, deliverables: '', dueDate: (form.getFieldValue('dueDate') as string) || '', amount: '0', paid: false, sortOrder: field.state.value.length })} className="border border-[#201e1d] bg-white px-3 py-1.5 text-xs font-semibold hover:bg-[#f0dcd8] rounded-none text-[#201e1d]">Add tranche</Button>
                    <Button
                      type="button"
                      onClick={() => {
                        const items = (form.getFieldValue('items') as any[]) || []
                        const subtotal = items.reduce((s: number, it: any) => s + Number(it.qty || 0) * Number(it.cost || 0), 0)
                        const n = field.state.value.length || 1
                        const each = Math.round((subtotal / n) * 100) / 100
                        const newTranches = field.state.value.map((t: any, i: number) => ({
                          ...t,
                          amount: i === n - 1 ? (Math.round((subtotal - each * (n - 1)) * 100) / 100).toFixed(2) : each.toFixed(2),
                        }))
                  ...
                  <div className="flex flex-col gap-[2px] bg-[#201e1d] border-2 border-[#201e1d] p-[2px]">
                    {(field.state.value || []).map((_, index) => (
                      <div key={index} className="bg-white p-3 grid grid-cols-[1fr_1.8fr_1fr_1fr_auto] gap-2 items-end">
                        <form.Field name={`tranches[${index}].name`}>
                          {(sub) => (
                            <Field>
                              <FieldLabel>Milestone</FieldLabel>
                              <Input value={sub.state.value} onChange={(e) => sub.handleChange(e.target.value)} onBlur={sub.handleBlur} placeholder="M1 — Mobilisation" className="h-9 px-2.5 text-[13px]" />
                              <FieldError errors={sub.state.meta.errors} />
                            </Field>
                          )}
                        </form.Field>
                        <form.Field name={`tranches[${index}].deliverables`}>
                          {(sub) => (
                            <Field>
                              ...
                              <Input value={sub.state.value || ''} onChange={(e) => sub.handleChange(e.target.value)} onBlur={sub.handleBlur} placeholder="SOW signature, PO" className="h-9 px-2.5 text-[13px]" />
                              <FieldError errors={sub.state.meta.errors} />
                            </Field>
                          )}
                        </form.Field>
                        ...
                        <form.Field name={`tranches[${index}].amount`}>
                          {(sub) => (
                            <Field>
                              ...
                              <Input type="text" inputMode="decimal" value={sub.state.value} onChange={(e) => sub.handleChange(e.target.value)} onBlur={sub.handleBlur} placeholder="0.00" className="h-9 px-2.5 text-[13px] text-right font-mono tabular-nums" />
                              <FieldError errors={sub.state.meta.errors} />
                            </Field>
                          )}
                        </form.Field>
                        <div className="flex gap-1 self-end">
                          <form.Field name={`tranches[${index}].paid`}>
                            {(paidField) => (
                              <Button type="button" onClick={() => paidField.handleChange(!paidField.state.value)} className={`h-9 px-3 text-xs font-semibold rounded-none border-2 ${paidField.state.value ? 'bg-[#ec3013] text-white border-[#ec3013]' : 'bg-white text-[#201e1d] border-[#201e1d] hover:bg-[#f0dcd8]'}`}>{paidField.state.value ? 'Paid' : 'Unpaid'}</Button>
                            )}
                          </form.Field>
...
function TrancheSummary({ form }: { form: InvoiceFormApi }) {
  return (
    <form.Subscribe selector={(s) => [s.values.items, s.values.currency, s.values.taxName, s.values.taxRate, s.values.tranches] as const}>
      {([items, currency, taxName, taxRate, tranches]) => {
        const subtotal = (items as any[]).reduce((s: number, it: any) => s + Number(it.qty || 0) * Number(it.cost || 0), 0)
        const rate = Number(taxRate || 0)
        const tax = subtotal * (rate / 100)
        const total = subtotal + tax
        const trSum = (tranches as any[] | undefined)?.reduce((s: number, t: any) => s + Number(t.amount || 0), 0) ?? 0
        const hasWarn = Math.abs(trSum - subtotal) > 0.5 && (tranches as any[])?.length > 0
        const fmt = (n: number) => {
          try { return new Intl.NumberFormat('en-NG', { style: 'currency', currency: (currency as string) || 'NGN', minimumFractionDigits: 2 }).format(n) } catch { return `${currency || 'NGN'} ${n.toFixed(2)}` }
        }
        return (
          <>
            {hasWarn && <div className="mt-2 text-xs font-semibold text-[#c02a10]">Tranches total {fmt(trSum)} — subtotal is {fmt(subtotal)}</div>}
            <div className="flex justify-end gap-6 mt-4 pt-3 border-t-2 border-[#201e1d] text-[13px]">
              <div>Subtotal <strong className="tabular-nums">{fmt(subtotal)}</strong></div>
              <div>{(taxName as string) || 'VAT'} ({rate}%) <strong className="tabular-nums">{fmt(tax)}</strong></div>
              <div>Total <strong className="tabular-nums">{fmt(total)}</strong></div>
            </div>
          </>
        )
      }}
    ...
    <form.Field name="paymentMethod">
      {(pmField) => (
        <div>
          ...
          <div className="flex gap-2 mb-3">
            <Button type="button" variant={pmField.state.value === 'bank' ? 'default' : 'outline'} onClick={() => pmField.handleChange('bank')} className={`rounded-none border-2 px-4 py-1.5 text-xs font-semibold ${pmField.state.value === 'bank' ? 'bg-[#201e1d] text-white border-[#201e1d]' : 'bg-white text-[#201e1d] border-[#201e1d] hover:bg-[#f0dcd8]'}`}>Bank account on the invoice</Button>
            <Button type="button" variant={pmField.state.value === 'link' ? 'default' : 'outline'} onClick={() => pmField.handleChange('link')} className={`rounded-none border-2 px-4 py-1.5 text-xs font-semibold ${pmField.state.value === 'link' ? 'bg-[#201e1d] text-white border-[#201e1d]' : 'bg-white text-[#201e1d] border-[#201e1d] hover:bg-[#f0dcd8]'}`}>Payment link</Button>
          </div>
                ...
                <form.Field name="payLinkLabel">
                  {(field) => (
                    <Field>
                      ...
                      <Input value={field.state.value || ''} onChange={(e) => field.handleChange(e.target.value)} onBlur={field.handleBlur} placeholder="Pay online" />
                      <FieldError errors={field.state.meta.errors} />
                    </Field>
                  )}
                </form.Field>
              ...
              <form.Field name="payLinkCurrency">
                {(field) => (
                  <Field>
                    ...
                    <CurrencySelect value={field.state.value || ''} onValueChange={(v) => field.handleChange(v)} placeholder="Select currency" />
                    <FieldError errors={field.state.meta.errors} />
                  </Field>
                )}
              </form.Field>
          ...
          <div className="mt-4">
            <form.Field name="memo">
              {(field) => (
                <Field>
                  <FieldLabel>Editable note printed at the foot of the invoice</FieldLabel>
                  <Textarea value={field.state.value || ''} onChange={(e) => field.handleChange(e.target.value)} onBlur={field.handleBlur} placeholder="Withholding tax of 5% applies per clause 5.5 ..." rows={3} />
                  <FieldError errors={field.state.meta.errors} />
                </Field>
              )}
            </form.Field>
          ...
          <CommandList>
            ...
            <CommandGroup>
              {banksData?.banks?.map((b: any) => (
                <CommandItem key={b.id} value={b.label} onSelect={() => field.handleChange(b.id)} className="rounded-none" data-checked={field.state.value === b.id}>
                  <div className="grid">
                    <span className="font-medium">{b.label}</span>
                    <span className="text-xs text-[#5c5755]">{b.currency} • {b.fields.map(([k, v]: [string, string]) => `${k}: ${v}`).join(' • ')}</span>
                  </div>
                </CommandItem>
              ))}
            </CommandGroup>
