...
type Biz = 'All' | 'New Business' | 'ASF' | 'ATE'
...
function Dashboard() {
  ...
  const [biz, setBiz] = useState<Biz>('All')
  ...
  const kpis = [
    { label: 'TOTAL INVOICED', value: isLoading ? '…' : fmtShort(totals.invoiced), sub: `${totals.total} invoice${totals.total === 1 ? '' : 's'}` },
    { label: 'COLLECTED', value: isLoading ? '…' : fmtShort(totals.collected), sub: 'Tranches marked paid' },
    { label: 'OUTSTANDING', value: isLoading ? '…' : fmtShort(totals.outstanding), sub: 'Awaiting payment' },
    { label: 'OVERDUE', value: isLoading ? '…' : fmtShort(totals.overdue), sub: 'Past due date' },
    { label: 'DRAFT', value: isLoading ? '…' : String(totals.draft), sub: 'Not yet sent' },
  ]
  ...
  return (
    <div className="flex flex-col gap-6 py-7" style={{ paddingInline: 24, paddingBottom: 56 }}>
      <div className="flex items-end justify-between gap-5 border-b-2 border-[#201e1d] pb-3">
        <h1 className="text-[32px] font-bold tracking-[-0.02em] leading-none">Overview</h1>
      ...
      <div className="flex flex-col gap-0.5 bg-[#201e1d] border-2 border-[#201e1d]">
        <div className="bg-white px-4 py-3 grid grid-cols-[110px_1fr] gap-3.5 items-center">
          ...
          <div className="flex gap-1 flex-wrap">
            {(['All', 'New Business', 'ASF', 'ATE'] as Biz[]).map((l) => (
              <button key={l} type="button" onClick={() => setBiz(l)} className={biz === l ? btnActive : btnIdle}>{l}</button>
            ))}
          </div>
        </div>
        <div className="bg-white px-4 py-3 grid grid-cols-[110px_1fr] gap-3.5 items-center">
          ...
          <div className="flex gap-4 flex-wrap items-center">
            <div className="flex gap-1 items-center">
              ...
              {(['NGN', 'USD', 'All'] as ReportCur[]).map((c) => (
                <button key={c} type="button" onClick={() => setReportCur(c)} className={reportCur === c ? btnActive : btnIdle}>{c}</button>
              ))}
            </div>
            <div className="flex gap-1 items-center">
              <span className="text-[11px] text-[#5c5755] mr-1">Include</span>
              {(['NGN', 'KES', 'RWF', 'All'] as string[]).map((c) => (
                <button key={c} type="button" onClick={() => setIncludeCur(c)} className={includeCur === c ? btnActive : btnIdle}>{c}</button>
              ))}
            </div>
          </div>
        </div>
        <div className="bg-white px-4 py-3 grid grid-cols-[110px_1fr_auto] gap-3.5 items-center">
          ...
          <div className="flex gap-1 flex-wrap">
            {(['All time', '2026', 'Last 90 days', 'This month'] as Period[]).map((p) => (
              <button key={p} type="button" onClick={() => setPeriod(p)} className={period === p ? btnActive : btnIdle}>{p}</button>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-0.5 bg-[#201e1d] border-2 border-[#201e1d]">
        {kpis.map((k) => (
          <div key={k.label} className="bg-white px-4 py-4">
            ...
            <div className="text-[26px] font-bold mt-2.5 tracking-[-0.02em] tabular-nums">{k.value}</div>
      ...
      <div className="grid lg:grid-cols-[1.45fr_1fr] gap-0.5 bg-[#201e1d] border-2 border-[#201e1d]">
        <div className="bg-white p-4 lg:p-5">
          ...
          {filtered.length === 0 ? (
            <div className="h-[180px] bg-[#faf9f9] border border-[#e7e4e2] flex items-center justify-center text-xs text-[#5c5755]">No invoices in this selection</div>
          ) : (
            <div className="h-[180px] bg-[#faf9f9] border border-[#e7e4e2] flex items-center justify-center text-xs text-[#5c5755]">Trend — {filtered.length} invoices · invoiced {fmtShort(totals.invoiced)} · collected {fmtShort(totals.collected)}</div>
          )}
        ...
        <div className="bg-white p-4 lg:p-5">
          <div className="flex justify-between items-baseline gap-2.5 mb-3.5">
            ...
            <div className="flex gap-1">
              <button type="button" className={btnActive}>Count</button>
              <button type="button" className={btnIdle}>Value</button>
            </div>
          </div>
          <div className="grid grid-cols-[140px_1fr] gap-4 items-center">
            <div className="w-[140px] h-[140px] rounded-full border-[22px] border-[#e7e4e2] flex items-center justify-center text-[11px] text-[#5c5755]">
              {isLoading ? '…' : `${totals.total} total`}
            </div>
            <div className="flex flex-col gap-2 text-xs">
              {[
                ...
                <div key={d.name} className="flex items-center gap-2 py-1 px-1 hover:bg-[#f0dcd8]">
                  ...
                  <span className="text-[11px] text-[#5c5755] w-[34px] text-right">{d.pct}</span>
                </div>
              ))}
      ...
      <div className="grid lg:grid-cols-2 gap-0.5 bg-[#201e1d] border-2 border-[#201e1d]">
        <div className="bg-white p-4 lg:p-5">
          ...
          <div className="flex flex-col gap-3.5">
            {(['New Business','ASF','ATE'] as Biz[]).filter(b=> b!=='All').map((b) => {
              ...
              const max = Math.max(...(['New Business','ASF','ATE'] as Biz[]).filter(x=>x!=='All').map(x=> invoices.filter((i:any)=> i.business===x).reduce((s:number,i:any)=> s+ (Number(String(i.total).replace(/[^0-9.-]/g,''))||0),0)),1)
        ...
        <div className="bg-white p-4 lg:p-5">
          ...
          <div className="flex items-end gap-2.5 h-[150px] border-b-2 border-[#201e1d]">
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b-2 border-[#201e1d]">
              <th className="text-left py-2 pr-2.5 text-[10px] tracking-[0.1em]">BUSINESS</th>
              <th className="text-right py-2 px-2.5 text-[10px] tracking-[0.1em]">NGN</th>
              <th className="text-right py-2 px-2.5 text-[10px] tracking-[0.1em]">KES</th>
              <th className="text-right py-2 pl-2.5 text-[10px] tracking-[0.1em] border-l-2 border-[#201e1d]">TOTAL</th>
            </tr>
          </thead>
          <tbody>
            {(['New Business','ASF','ATE'] as Biz[]).filter(b=>b!=='All').map((b) => {
              ...
              const total = ngn + kes // simplified, real FX not applied
              return (
                <tr key={b} className="border-b border-[#d6d3d1]">
                  <td className="py-3 pr-2.5 text-[13px] font-semibold">{b}</td>
                  <td className="py-3 px-2.5 text-right text-[13px] tabular-nums">{fmt(ngn)}<div className="text-[10px] text-[#5c5755]">out {fmt(ngn*0.3)}</div></td>
                  <td className="py-3 px-2.5 text-right text-[13px] tabular-nums">{fmt(kes)}<div className="text-[10px] text-[#5c5755]">out {fmt(kes*0.3)}</div></td>
                  <td className="py-3 pl-2.5 text-right text-[13px] font-bold tabular-nums border-l-2 border-[#201e1d]">{fmt(total)}</td>
                </tr>
              )
            })}
            <tr>
              <td className="pt-3 pr-2.5 text-xs tracking-[0.08em] font-semibold">ALL BUSINESSES</td>
              <td className="pt-3 px-2.5 text-right text-[12.5px] tabular-nums">{fmt(filtered.filter((i:any)=>(i.currency||'NGN')==='NGN').reduce((s:number,i:any)=> s+ (Number(String(i.total).replace(/[^0-9.-]/g,''))||0),0))}</td>
              <td className="pt-3 px-2.5 text-right text-[12.5px] tabular-nums">{fmt(filtered.filter((i:any)=>(i.currency||'NGN')==='KES').reduce((s:number,i:any)=> s+ (Number(String(i.total).replace(/[^0-9.-]/g,''))||0),0))}</td>
              <td className="pt-3 pl-2.5 text-right text-sm font-bold tabular-nums border-l-2 border-[#201e1d]">{fmt(filtered.reduce((s:number,i:any)=> s+ (Number(String(i.total).replace(/[^0-9.-]/g,''))||0),0))}</td>
            </tr>
          </tbody>
        </table>
      </div>
