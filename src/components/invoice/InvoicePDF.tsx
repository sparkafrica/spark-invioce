      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.headerRow}>
          ...
          <View style={styles.invoiceMeta}>
            ...
            <Text style={styles.invoiceDate}>Date: {data.invoice.issueDate}</Text>
          </View>
        </View>
        ...
        <View style={styles.grid2}>
          <View style={styles.grid2Col}>
            ...
            {data.company.reg && <Text style={styles.normalText}>{data.company.reg}</Text>}
            {data.company.address && <Text style={styles.normalText}>{data.company.address}</Text>}
            {data.company.tin && <Text style={styles.normalText}>TIN: {data.company.tin}</Text>}
          </View>
          <View style={styles.grid2Col}>
            <Text style={styles.kicker}>BILL TO</Text>
            <Text style={styles.fromName}>{data.client.name}</Text>
            {data.client.reg && <Text style={styles.normalText}>{data.client.reg}</Text>}
            {data.client.address && <Text style={styles.normalText}>{data.client.address}</Text>}
            {data.client.email && <Text style={styles.normalText}>{data.client.email}</Text>}
          </View>
        </View>
        ...
        <View style={styles.table}>
          <View style={styles.tableHeaderRow}>
            ...
            <Text style={[styles.th, styles.colDeliverables]}>DELIVERABLES</Text>
            <Text style={[styles.th, styles.colDue]}>DUE</Text>
            <Text style={[styles.th, styles.colAmount, styles.thRight]}>AMOUNT ({data.invoice.currency})</Text>
            <Text style={[styles.th, styles.colTax, styles.thRight]}>{data.invoice.taxName} {taxRate ? `${taxRate}%` : ''}</Text>
            <Text style={[styles.th, styles.colTotal, styles.thRight]}>TOTAL ({data.invoice.currency})</Text>
          </View>
          {lines.map((l, i) => {
            const isLast = i === lines.length - 1
            return (
              <View key={i} style={[styles.tdRow, isLast ? styles.tdRowLast : {}]}>
                <Text style={[styles.td, styles.tdBold, styles.colMilestone]}>{l.name}</Text>
                <Text style={[styles.td, styles.colDeliverables]}>{l.deliverables}</Text>
                <Text style={[styles.td, styles.colDue]}>{l.due}</Text>
                <Text style={[styles.td, styles.tdTabular, styles.colAmount]}>{l.amount}</Text>
                <Text style={[styles.td, styles.tdTabular, styles.colTax]}>{l.tax}</Text>
                <Text style={[styles.td, styles.tdBold, styles.tdTabular, styles.colTotal]}>{l.total}</Text>
              </View>
            )
          })}
        </View>
        ...
        <View style={styles.totalsGrid}>
          ...
          <View style={styles.totalsBlock}>
            <View style={styles.totalRow}>
              ...
              <Text style={styles.totalValue}>{fmt(data.subtotal, data.invoice.currency)}</Text>
            </View>
            <View style={styles.totalRow}>
              <Text style={styles.totalLabel}>
                {data.invoice.taxName} ({data.invoice.taxRate}%)
              </Text>
              <Text style={styles.totalValue}>{fmt(data.taxAmount, data.invoice.currency)}</Text>
            </View>
            <View style={styles.grandTotalRow}>
              <Text style={styles.grandLabel}>Total due</Text>
              <Text style={styles.grandValue}>{fmt(data.total, data.invoice.currency)}</Text>
            </View>
        ...
        <View style={styles.paymentGrid}>
          <View style={styles.paymentCol}>
            <Text style={styles.paymentKicker}>PAYMENT INFORMATION — BANK TRANSFER</Text>
            <Text style={styles.paymentText}>Bank: {data.bank?.label || '—'}</Text>
            {data.bank?.fields.map(([k, v]) => (
              <Text key={k} style={styles.paymentText}>
                {k}: {v}
              </Text>
            ))}
            {data.company.tin && <Text style={styles.paymentText}>TIN: {data.company.tin}</Text>}
            {data.invoice.paymentMethod === 'link' && data.invoice.payLink && (
              <Text style={styles.paymentText}>
                {data.invoice.payLinkLabel || 'Pay link'}: {data.invoice.payLink}
              </Text>
            )}
          </View>
          ...
          <View style={styles.paymentsSection}>
            ...
            {data.payments.map((p, idx) => (
              <View key={idx} style={styles.paymentRow}>
                ...
                <Text style={styles.paymentAmount}>{fmt(Number(p.amount), data.invoice.currency)}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        {isVoided && (
          <View style={{ marginTop: 12, alignItems: 'center' }}>
            <Text style={[styles.memoText, { color: '#c02a10', fontSize: 9 }]}>This invoice has been voided.</Text>
          </View>
        )}
      </Page>
    </Document>
