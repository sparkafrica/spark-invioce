...
export function InvoiceForm({
  ...
  const defaultValues = {
    ...
    tranches: (initialData?.tranches?.length ? initialData.tranches : defaultTranches).map(
      normalizeTranche,
    ),
  ...
  const paymentMutation = useMutation({
    ...
    onError: (e: unknown) => {
      toast.add({ description: getErrorMessage(e, 'Failed to record payment'), type: 'error' });
    },
  });
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        const normalizedTranches = (value.tranches ?? []).map((tranche, index) => ({
          ...tranche,
          sortOrder: tranche.sortOrder ?? index,
        }));
        const payload = {
  ...
  useEffect(() => {
    ...
    const nextValues = {
      ...
      payLinkCurrency: initialData.payLinkCurrency ?? defaultValues.payLinkCurrency,
      items: (initialData.items?.length ? initialData.items : defaultItems).map(
        normalizeItem,
      ),
      tranches: (initialData.tranches?.length ? initialData.tranches : defaultTranches).map(
        ...
        <DialogContent className="rounded-none">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              variant="default"
              onClick={() => {
                (form as unknown as { setFieldValue: (name: string, value: string) => void }).setFieldValue('number', suggestedNumber);
