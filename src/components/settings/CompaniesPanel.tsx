...
export function CompaniesPanel() {
	...
	const companies =
		(data as unknown as { companies?: unknown[] } | undefined)?.companies ?? [];

	return (
		<div className="flex flex-col gap-4">
			<div className="text-[10px] tracking-[0.12em] font-semibold text-[#c02a10]">
				INVOICING COMPANIES — {companies.length} total
			</div>
			<div className="flex flex-col gap-3">
				{(companies as Array<Record<string, unknown> & { id: string }>).map(
					(c) => (
