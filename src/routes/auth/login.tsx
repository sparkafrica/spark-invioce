...
function LoginPage() {
  ...
  const form = useForm({
    ...
    validators: {
      onChange: ({ value }) => standardSchemaValidators.validate({ value, validationSource: 'field' }, loginSchema),
      onSubmit: ({ value }) => standardSchemaValidators.validate({ value, validationSource: 'form' }, loginSchema),
    },
    onSubmit: async ({ value }) => {
      ...
      if (error) toast.add({ description: error?.message ?? 'Login failed', type: 'error' })
    },
  })
      ...
      <div className="flex items-center px-6 py-12 lg:px-14">
        <div className="w-full max-w-90 mx-auto flex flex-col gap-4">
          ...
          >
            ...
            <Link to="/auth/forgot-password" className="w-full border border-[#201e1d] bg-transparent px-3.5 py-3 text-left text-xs font-semibold hover:bg-[#f0dcd8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#ec3013]">Forgot password — reset it</Link>
          </form>
