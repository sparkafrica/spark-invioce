...
import { resetPassword } from '#/lib/server-fns/auth';
...
export const Route = createFileRoute('/_auth/auth/reset-password')({
	validateSearch: (search: Record<string, unknown>) =>
		v.parse(v.object({ token: v.optional(v.string()) }), search),
	...
	errorComponent: ({ error }) => (
		<div className="min-h-screen bg-[#f3f2f2] text-[#201e1d] flex items-center justify-center p-6">
			<div className="w-full max-w-90 border-2 border-[#201e1d] bg-white p-6">
				...
				<p className="mt-2 text-xs leading-5 text-[#5c5755]">
					{getErrorMessage(error, 'Something went wrong')}
				</p>
				<Link
					to="/auth/forgot-password"
					className="mt-4 inline-block border border-[#201e1d] px-3 py-2 text-xs font-semibold hover:bg-[#f0dcd8]"
				>
			...
			<div className="min-h-screen grid lg:grid-cols-2">
				...
				<div className="flex items-center px-6 py-12 lg:px-14">
					<div className="w-full max-w-90 mx-auto flex flex-col gap-4">
						...
						{errorMsg && !success && (
							<div
								className="border border-[#c02a10] bg-[#f0dcd8] p-3 text-xs font-semibold text-[#c02a10]"
								role="alert"
							>
								{errorMsg}
							</div>
