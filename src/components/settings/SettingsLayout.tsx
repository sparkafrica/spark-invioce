...
const TABS = [
	...
	},
	{
		...
		href: '/settings/companies' as const,
	},
	{ id: 'banks', label: 'Bank accounts', href: '/settings/banks' as const },
	{
		id: 'businesses',
		label: 'Businesses & logos',
		href: '/settings/businesses' as const,
	},
	{
		id: 'fx-rates',
		label: 'Exchange Rates',
		href: '/settings/fx-rates' as const,
	},
] as const;
...
export function SettingsLayout() {
	...
	const isAdmin =
		...
		(session as any)?.session?.userRole === 'admin';
	const isAdminDemo =
		session?.user?.email === 'clinton@sparkafrica.co' ||
		session?.user?.email === 'admin@spark.com';
	...
	const activeId =
		TABS.find((t) => path === t.href)?.id ||
		(path === '/settings' || path === '/settings/' ? 'profile' : 'profile');

	const btnActive =
		'bg-[#201e1d] text-white border border-[#201e1d] px-3 py-1.5 text-xs font-semibold';
	const btnIdle =
		'bg-white text-[#201e1d] border border-[#201e1d] px-3 py-1.5 text-xs font-semibold hover:bg-[#f0dcd8]';
				...
				<div className="flex gap-1 flex-wrap">
					{TABS.map((t) => (
						<Link
							key={t.id}
							to={t.href}
							className={activeId === t.id ? btnActive : btnIdle}
						>
							{t.label}
						</Link>
					))}
