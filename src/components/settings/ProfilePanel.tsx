...
export function ProfilePanel() {
  ...
  const form = useForm({
    defaultValues: {
      ...
      title: ((session?.user as unknown as { title?: string })?.title as string) ?? '',
    },
    validators: {
      onChange: ({ value }) => standardSchemaValidators.validate({ value, validationSource: 'field' }, profileSchema),
      onSubmit: ({ value }) => standardSchemaValidators.validate({ value, validationSource: 'form' }, profileSchema),
    },
    onSubmit: async ({ value }) => {
      try {
        // Real mutation: better-auth updateUser (replaces stub `async (data)=>data`)
        const res = (await (authClient.updateUser as unknown as (d: Record<string, unknown>) => Promise<{ error?: { message?: string } }>)({
          name: value.name,
          // additionalFields `title` declared in auth.ts user config
          title: value.title,
          // `image` supported by better-auth; keep undefined if not edited
          image: (session?.user as unknown as { image?: string })?.image ?? undefined,
  ...
  }, [session?.user?.name, (session?.user as unknown as { title?: string })?.title])
      ...
      >
        <div className="grid gap-3 md:grid-cols-2">
          ...
          <Field>
            ...
            <Input value={session?.user?.email ?? ''} disabled className="bg-[#f3f2f2]" />
          </Field>
        </div>
