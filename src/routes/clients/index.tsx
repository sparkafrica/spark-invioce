    ...
    return (
      <div className="grid lg:grid-cols-[1.5fr_1fr] gap-9 items-start">
        ...
        <div className="border-l-2 border-[#201e1d] pl-7">
          <div className="text-[10px] tracking-[0.12em] font-semibold mb-3">ADD A CLIENT</div>
              ...
              <Table className="bg-white">
                <TableHeader className="bg-[#f3f2f2]">
                  <TableRow className="border-b-2 border-[#201e1d] hover:bg-transparent">
                    <TableHead className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#c02a10] h-auto">NAME</TableHead>
                    <TableHead className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#c02a10] h-auto">CONTACT</TableHead>
                    <TableHead className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#c02a10] h-auto">EMAIL</TableHead>
                    <TableHead className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wider text-[#c02a10] h-auto">REGISTRATION</TableHead>
                ...
                <TableBody className="divide-y divide-[#d6d3d1]">
                  {filteredClients.map((client) => (
                    <TableRow key={client.id} className="hover:bg-[#f0dcd8]">
                      ...
                      <TableCell className="px-4 py-3 text-[#5c5755]">{client.contact || '-'}</TableCell>
                      <TableCell className="px-4 py-3 text-[#5c5755]">{client.email || '-'}</TableCell>
                      <TableCell className="px-4 py-3 text-[#5c5755]">{client.reg || '-'}</TableCell>
                      <TableCell className="px-4 py-3 text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button variant="ghost" size="icon" onClick={() => setEditingId(client.id)}>
                            <EditIcon className="h-4 w-4" />
                          </Button>
                          <Button variant="ghost" size="icon" className="text-[#c02a10]" onClick={() => deleteMutation.mutate(client.id)} disabled={deleteMutation.isPending}>
