...
function SeedPage() {
  ...
  const handleSendToken = async () => {
    ...
    try {
      await sendSeedToken()
      toast.add({ title: 'Token sent', description: 'Check your email for the 6-digit seed token', type: 'success' })
      setStage('verify')
    } catch (e: any) {
      toast.add({ description: e.message || 'Failed to send token', type: 'error' })
    } finally {
      setSending(false)
    }
  }
  ...
  const handleVerifyToken = async () => {
    ...
    try {
      ...
      if (result.valid) {
        toast.add({ title: 'Token verified', description: 'Seeding database...', type: 'success' })
        await seedDatabase()
      } else {
        toast.add({ title: 'Invalid token', description: 'Token is invalid or expired', type: 'error' })
      }
    } catch (e: any) {
      toast.add({ description: e.message || 'Verification failed', type: 'error' })
    } finally {
      setVerifying(false)
    }
  }
  ...
  const seedDatabase = async () => {
    ...
    try {
      ...
      toast.add({ title: 'Seeded successfully', description: `Organization ${result.organizationId} created`, type: 'success' })
      window.location.href = '/auth/login'
    } catch (e: any) {
      toast.add({ title: 'Seed failed', description: e.message || 'Database seeding failed', type: 'error' })
    } finally {
      setSeeding(false)
    }
  }
  ...
  const handleClearDb = async () => {
    ...
    try {
      await clearDb()
      toast.add({ title: 'Database cleared', description: 'All data has been removed', type: 'success' })
      ...
      <div className="w-full max-w-560 bg-white border-2 border-[#201e1d] p-6">
        ...
        {needsOrgId && (
          <div className="mb-6 p-4 bg-[#fff2ef] border border-[#ec3013] rounded-none">
            <p className="text-[13px] text-[#201e1d] mb-3">
              <strong>Organization ID not configured.</strong> Add <code className="bg-[#f3f2f2] px-1.5 py-0.5 font-mono text-[11px]">ORGANIZATION_ID=your-org-id</code> to <code className="bg-[#f3f2f2] px-1.5 py-0.5 font-mono text-[11px]">.env.local</code> and redeploy before seeding.
            </p>
            <Button
              variant="ghost"
              size="sm"
        ...
        <div className="text-[13px] text-[#5c5755] mb-6 leading-relaxed">
          ...
          <ul className="list-disc list-inside space-y-1 text-[12px] mb-3">
            <li>3 businesses: New Business (SPK), Africa Startup Festival (ASF), Africa Technology Expo (ATE)</li>
          ...
          <p className="text-[11px] font-semibold text-[#c02a10]">
            Demo users: clinton@sparkafrica.co (owner), kingsonseang@gmail.com (admin), ada@sparkafrica.co (editor), tolu@sparkafrica.co (invited). Password for all: <code className="font-mono">spark</code>
          </p>
        </div>
        ...
        {stage === 'verify' && (
          <div className="space-y-3">
            <div className="flex flex-col gap-1">
              <Label className="text-[11px] font-semibold text-[#201e1d]">Enter 6-digit token</Label>
              <Input
                type="text"
                value={token}
