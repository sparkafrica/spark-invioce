...
const fxSchema = v.object({
	...
	rates: v.record(
		v.string(),
		v.pipe(v.number(), v.minValue(0, 'Rate must be >= 0')),
	),
});
...
export function FXRatesPanel({ canManage }: { canManage: boolean }) {
	...
	const raw = (data as unknown as { fxRates?: typeof defaultFx } | undefined)
		?.fxRates;
	...
	const form = useForm({
		...
		onSubmit: async ({ value }) => {
			try {
				...
				const msg =
					e instanceof Error ? e.message : 'Failed to update FX rates';
						...
						<Field>
							...
							<RadioGroup
								...
								onValueChange={(val) => {
									if (isEditing && canManage)
										field.handleChange(val as 'manual' | 'api');
								}}
							...
							{field.state.value === 'api' && (
								<div className="text-xs text-[#5c5755] bg-[#fff2ef] border border-[#ec3013] p-3">
									Rates fetched daily via exchangerate-api.com. Requires valid{' '}
									<code className="font-mono">EXCHANGERATE_API_KEY</code> in
									environment. Only available if API is accessible.
								</div>
							)}
						</Field>
					)}
				...
				<form.Subscribe selector={(s) => s.values.mode}>
					{(mode) => (
						<div className="grid gap-2 md:grid-cols-2 lg:grid-cols-4">
							{Object.entries(
								(form.getFieldValue('rates') as Record<string, number>) ??
									fxData.rates,
							).map(([code]) => (
