...
export function InvoiceTable({ data, onView, onEdit }: InvoiceTableProps) {
  const [sorting, setSorting] = useState<import('@tanstack/table-core').SortingState>([])
  const [columnFilters, setColumnFilters] = useState<import('@tanstack/table-core').ColumnFiltersState>([])
  const [globalFilter, setGlobalFilter] = useState('')
  const [pagination, setPagination] = useState<import('@tanstack/table-core').PaginationState>({
    pageIndex: 0,
    pageSize: 10,
  })
  ...
  const columns = useMemo(() => {
    ...
    return [
      h.accessor('number', {
        header: 'NUMBER',
        cell: (info) => <span className="font-semibold whitespace-nowrap text-[13px]">{info.getValue()}</span>,
      }),
      h.accessor('client', {
        header: 'CLIENT',
      ...
      h.accessor('type', {
        header: 'TYPE',
        cell: (info) => <span className="whitespace-nowrap text-xs capitalize">{info.getValue()}</span>,
      }),
      h.accessor('total', {
        header: 'TOTAL',
        cell: (info) => <span className="tabular-nums text-right block font-semibold text-[13px]">{info.getValue()}</span>,
      }),
      h.accessor('status', {
        header: 'STATUS',
        cell: (info) => {
          const s = info.getValue()
          const chip = 'inline-block whitespace-nowrap px-2 py-1 text-[11px] font-semibold'
          const style =
            s === 'paid'
      ...
      h.accessor('commentCount', {
        header: 'NOTES',
        cell: (info) => <span className="text-[11px] text-[#5c5755] whitespace-nowrap">{info.getValue()}</span>,
      }),
      h.display({
        id: 'actions',
        header: '',
    ...
    <div className="flex flex-col gap-5">
              ...
              table.getRowModel().rows.map((row) => (
                <TableRow
                  ...
                  className={cn(
                    ...
                    (row.original as Invoice).status === 'voided' && 'opacity-60'
                  )}
                >
      ...
      <div className="flex items-center justify-between">
        <div className="text-xs text-[#5c5755]">
          ...
          {Math.min((table.state.pagination.pageIndex + 1) * table.state.pagination.pageSize, table.getFilteredRowModel().rows.length)} of{' '}
