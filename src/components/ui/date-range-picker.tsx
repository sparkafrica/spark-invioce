...
const PRESETS: Array<{ label: string; value: string; getRange: () => { from: Date; to: Date } }> = [
  {
    label: 'Today',
    value: 'today',
    getRange: () => {
...
export function DateRangePicker({ value, onChange, placeholder = 'Select date range', disabled, className }: DateRangePickerProps) {
  ...
  const handleDayClick = useCallback((range: { from?: Date; to?: Date } | undefined) => {
    if (!range || !range.from) {
